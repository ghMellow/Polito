...
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
  if ( timer_num == 0 )
  {
		...
		if(MatchReg == 0) {
			...
			LPC_TIM0->MCR |= SRImatchReg << (3*MatchReg); //bit wise OR così da usare in contemporanea tutti i match register senza pulire il registro
		} else if (MatchReg == 1) {
  ...
  {	
		...
		if(MatchReg == 0) {
			...
			LPC_TIM1->MCR |= SRImatchReg << (3*MatchReg); //bit wise OR così da usare in contemporanea tutti i match register senza pulire il registro
		} else if (MatchReg == 1) {
  ...
  {	
		...
		if(MatchReg == 0) {
			...
			LPC_TIM2->MCR |= SRImatchReg << (3*MatchReg); //bit wise OR così da usare in contemporanea tutti i match register senza pulire il registro
		} else if (MatchReg == 1) {
  ...
  {	
		...
		if(MatchReg == 0) {
			...
			LPC_TIM3->MCR |= SRImatchReg << (3*MatchReg); //bit wise OR così da usare in contemporanea tutti i match register senza pulire il registro
		} else if (MatchReg == 1) {
